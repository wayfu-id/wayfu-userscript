...
async function checkStatus() {
    if (loop.isRunning) {
        ...
        return (await modal.confirm("Stop Proses?"))
            ? (stopProcess(), true)
            : (startProcess(), true);
    }
    ...
    let blast = {
        message: {
            // value: DOM.getElement("#message").value,
            rules: ["notEmpty"],
        },
        queue: {
            // value: queue,
            rules: ["notEmpty", "belowMax"],
        },
        chatroom: {
            // value: chat,
            rules: ["notEmpty"],
    ...
    let isValid = validator.validate(blast);
    if (!isValid) {
        validator.showError();
    }
...
function printLink(no, link) {
    if (options.autoMode) {
        DOM.setElement(queryElm.linkElm, {
            href: `https://${link}`,
        });
        return true;
    }
    DOM.eventFire(queryElm.input, "input", `${no}). ${link}`);
    let sendBtn = DOM.getElement(queryElm.send);
    return sendBtn ? (sendBtn.click(), true) : false;
}
...
function initMessage() {
    const imgFile =
        options.imageFile || DOM.getElement("#imgFile").files.length !== 0
            ? DOM.getElement("#imgFile").files[0]
            : "";
    message.setProperties({
        inputMessage: DOM.getElement("#message").value,
        inputCaption: DOM.getElement("#_caption").value,
        imageFile: imgFile,
    });
}
...
function startProcess() {
    ...
    if (loop.isRunning && !!queue.now) {
        ...
        if (options.autoMode) {
            setTimeout(() => {
                DOM.getElement(queryElm.linkElm).click();
            }, 5e2);
            setTimeout(async () => {
                const err = DOM.getElement(queryElm.errModal);
                let stat = "";
                if (err) {
                    ...
                    const { useCaption, useImage, hasImage } = options;
                    const btn = DOM.getElement(queryElm.send);
                    ...
                    if (
                        useImage &&
                        hasImage &&
                        (useCaption !== "caption" || stat === "SUCCESS")
                    ) {
            ...
            /**
             * Doesn't need to go back to first chatroom
             * Link doesn't printed in chatroom Since version 4.0
             */
...
function showReport() {
    ...
    let title = "[REPORT] Penulisan Link Selesai.",
        text = {
            html: `${report.sukses} Link Berhasil Ditulis`,
            classid: "wfu-reports",
        };

    if (options.autoMode) {
        title = "[REPORT] Kirim Pesan Otomatis Selesai.";
...
function updateUI(index = 0.5, phone = null) {
    let num = queue.size,
        display = !num ? "none" : "inline-block",
        title = num ? `Data: Loaded, ${num} Nomor` : "";
    DOM.setElements([
        {
            elm: "#_file-info",
            props: { title: title, style: `display:${display}` },
        },
